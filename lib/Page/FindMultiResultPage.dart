...
class FindMultiResultPage extends StatefulWidget {
  ...
  State<FindMultiResultPage> createState() =>
      _FindMultiResultPageState(todoufuken, shichoson, gender, rank, age,blockList);
}

class _FindMultiResultPageState extends State<FindMultiResultPage> {
  _FindMultiResultPageState(
  ...
  Stream<List<QueryDocumentSnapshot>> _getSearchResultsListStream() {
    ...
    if (todoufuken != '') {
      if (shichoson != '') {
        query = query.where('TODOFUKEN_SHICHOSON_LIST', arrayContains: todoufuken + "(" + shichoson + ")");
      }else {
